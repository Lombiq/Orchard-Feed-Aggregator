...
namespace Lombiq.FeedAggregator.Services
{
    public class FeedSyncProfileUpdaterScheduledTask : IScheduledTaskHandler, IOrchardShellEvents
    {
        ...
        public void Process(ScheduledTaskContext context)
        {
            ...
            {
                // Swallowing all non-fatal exceptions so if a sync fails it won't be retried every minute. That would
                // happen if the exception would bubble up and thus also prevent the removal of the current task entry.

                Logger.Error
                    (ex, 
                    "Error during processing the FeedSyncProfile with the ID " + feedSyncProfileContentItem.Id + ".");
            }
        }
